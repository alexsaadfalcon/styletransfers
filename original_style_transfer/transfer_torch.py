...
print('using device', "cuda GPU" if torch.cuda.is_available() else "cpu")
...
def main(f_content="content/cat.jpg", f_style="style/oil.jpg", num_steps=300,
         content_weight=1, style_weight=1e6,
         content_layers=None, style_layers=None,
         plot=False):
    ######################################################################
    ...
    # these are cropped/rotated to be the same size


    content_img = image_loader(f_content)
    style_img = image_loader(f_style)
    ...
    output = run_style_transfer(content_img, style_img, input_img,
                                num_steps, content_weight, style_weight,
    ...
    output = output.cpu().clone()  # we clone the tensor to not do changes on it
